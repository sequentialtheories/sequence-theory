...
-- Removed CONCURRENTLY as it cannot run inside a transaction block
...
-- Made more flexible to support different wallet address formats
ALTER TABLE public.user_wallets 
ADD CONSTRAINT chk_wallet_address_format 
CHECK (length(wallet_address) >= 10 AND wallet_address ~ '^[a-zA-Z0-9]+$');